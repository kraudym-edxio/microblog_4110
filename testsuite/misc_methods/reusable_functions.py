...
from resc.config_files import settings
...
def from_shadow_dom_click_on_element(self, locator):
    shadow = Shadow(self.driver)
    shadow.find_element(locator).click()
...
def test_shadow_within_an_host(self, parent, child, iframe_locator = None):
    if iframe_locator is None:
        self.driver.switch_to.default_content()
    else:
        iframe = get_element(self.driver, iframe_locator)
        self.driver.switch_to.frame(iframe)

    shadow_host = self.driver.find_element(parent[0], parent[1])
    shadow_root = shadow_host.shadow_root

    time.sleep(2)
    shadow_content = shadow_root.find_element(child[0], child[1])

    actions = ActionChains(self.driver)
    hover = actions.click(shadow_content)
    hover.perform()
    # time.sleep(2)

    # content = shadow_root.find_element(By.CSS_SELECTOR, dock_locators.HOVER_DOCK[1])
    # assert content.text == "Hold to move"

def test_shadow_within_an_host_v2(self, parent, child, iframe_locator = None):
    if iframe_locator is None:
        self.driver.switch_to.default_content()
    else:
        iframe = get_element(self.driver, iframe_locator)
